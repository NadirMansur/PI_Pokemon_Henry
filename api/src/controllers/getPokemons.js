...
const getPokemons = async(req, res)=>{
    ...
    if(Object.keys(req.query).length > 0){
        try{
          console.log("entre al BUSCA POR QUERY");
          ...
          if(pokemon.length === 0){
            console.log("entre a hacer la peticion por pokeAPI")
            const nameLow = name.toLowerCase();
            const {data} = await axios.get(`https://pokeapi.co/api/v2/pokemon/${nameLow}`)
            const pokemon = {
              id: data.id,
          ...
          try{
            ...
            console.log("entre al  toDOS LOS POKEMONS DE LA BDD");
